...
def librosa_downsample(filename, outrate=8000):
    y, s = librosa.load(filename, sr=8000)
    return y, s

def custom_downsample(filename, outrate=8000):
    (rate, sig) = wav.read(filename)
    len_in_secs = len(sig)
    secs = len_in_secs/rate # Number of seconds in signal X
    samps = secs*outrate   # Number of samples to downsample
    print(secs, samps)
    Y = scipy.signal.resample(sig , int(samps))
    return Y, outrate
